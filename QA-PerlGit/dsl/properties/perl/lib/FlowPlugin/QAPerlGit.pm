...
sub getUserInfo {
    ...
    my $QAPerlGitRESTClient = $pluginObject->getQAPerlGitRESTClient;
    # If you have changed your parameters in the procedure declaration, add/remove them here
    my %restParams = (
    );
    my $response = $QAPerlGitRESTClient->getGitUserInfo(%restParams);
...
sub getQAPerlGitRESTClient {
    my ($self) = @_;

    my $context = $self->getContext();
    my $config = $context->getRuntimeParameters();
    require FlowPlugin::QAPerlGitRESTClient;
    my $client = FlowPlugin::QAPerlGitRESTClient->createFromConfig($config);
    return $client;
}
## === step ends ===
