...
sub getIssueTypes {
    ...
    my $QAJIRARESTClient = $pluginObject->getQAJIRARESTClient;
    # If you have changed your parameters in the procedure declaration, add/remove them here
    my %restParams = (
    );
    my $response = $QAJIRARESTClient->getIssueTypes(%restParams);
...
sub getQAJIRARESTClient {
    my ($self) = @_;

    my $context = $self->getContext();
    my $config = $context->getRuntimeParameters();
    require FlowPlugin::QAJIRARESTClient;
    my $client = FlowPlugin::QAJIRARESTClient->createFromConfig($config);
    return $client;
}
## === step ends ===
